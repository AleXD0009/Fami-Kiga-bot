...
const FILES_TO_RUN = ['main.js']
let activeWorkers = new Set()
let isRestarting = false

function start() {
if (cluster.isPrimary) {
FILES_TO_RUN.forEach(file => spawnWorker(file))
} else {
const fileToRun = process.env.FILE
if (fileToRun) {
import(path.join('file://', __dirname, fileToRun)).catch(err => { console.error(`Error al importar ${fileToRun}:`, err) })
}
}}
