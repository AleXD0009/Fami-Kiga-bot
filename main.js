...
if (!conn.authState.creds.registered) {
  ...
  if (conn.requestPairingCode) {
    let code = await conn.requestPairingCode(phoneNumber, 'FAMIKIGA')
...
const reason = lastDisconnect?.error?.output?.statusCode ?? lastDisconnect?.error?.statusCode ?? lastDisconnect?.error?.status;
if (connection === 'close') {
